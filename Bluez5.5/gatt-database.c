...
static void gatt_custom_cp_write_cb(struct gatt_db_attribute *attrib,
					unsigned int id, uint16_t offset,
...
static void gatt_custom_ft_read_cb(struct gatt_db_attribute *attrib,
					unsigned int id, uint16_t offset,
					uint8_t opcode, struct bt_att *att,
					void *user_data)
{
	...
	DBG("Test file transfer read callback of custom service and characteristics");
...
/* Test write callback of custom service and characteristics */
static void gatt_custom_ft_write_cb(struct gatt_db_attribute *attrib,
					unsigned int id, uint16_t offset,
					const uint8_t *value, size_t len,
					uint8_t opcode, struct bt_att *att,
					void *user_data)
{
	...
	DBG("Test file transfer write %s: %d %d %d %d %d\n", ecode ? "NG" : "OK", 
...
/* Test custom service and characteristics */
static void populate_test_service(struct btd_gatt_database *database)
{
	bt_uuid_t uuid;
	struct gatt_db_attribute *service;

	uint128_t test_srv = { .data = { 0x0a,0x0a,0x10,0x10,
										0x4e,0x41,
										0x42,0x49,
										0x5f,0x49,
										0x44,0x5f,0x42,0x41,0x53,0x45 } };
	uint128_t test_cp = { .data = { 0x0a,0x0a,0x10,0x11,
										0x4e,0x41,
										0x42,0x49,
										0x5f,0x49,
										0x44,0x5f,0x42,0x41,0x53,0x45 } };
	uint128_t test_ft = { .data = { 0x0a,0x0a,0x10,0x12,
										0x4e,0x41,
										0x42,0x49,
										0x5f,0x49,
										0x44,0x5f,0x42,0x41,0x53,0x45 } };
	uint128_t test_pm = { .data = { 0x0a,0x0a,0x10,0x14,
										0x4e,0x41,
										0x42,0x49,
										0x5f,0x49,
										0x44,0x5f,0x42,0x41,0x53,0x45 } };
	uint128_t test_ancs = { .data = { 0x0a,0x0a,0xe0,0x11,
										0x4e,0x41,
										0x42,0x49,
										0x5f,0x49,
										0x44,0x5f,0x42,0x41,0x53,0x45 } };

	bt_uuid128_create(&uuid, test_srv);
	service = gatt_db_add_service(database->db, &uuid, true, 5);
	...
	/*bt_uuid128_create(&uuid, test_ft);
	gatt_db_service_add_characteristic(service, &uuid, BT_ATT_PERM_READ | BT_ATT_PERM_WRITE,
							BT_GATT_CHRC_PROP_READ | BT_GATT_CHRC_PROP_WRITE,
							gatt_custom_ft_read_cb,
							gatt_custom_ft_write_cb, database);

	bt_uuid128_create(&uuid, test_ancs);
	gatt_db_service_add_characteristic(service, &uuid, BT_ATT_PERM_READ | BT_ATT_PERM_WRITE,
							BT_GATT_CHRC_PROP_READ | BT_GATT_CHRC_PROP_WRITE,
							gatt_custom_ft_read_cb,
							gatt_custom_ft_write_cb, database);*/
...
static void register_core_services(struct btd_gatt_database *database)
{
	...
	populate_test_service(database);//Test 
}
